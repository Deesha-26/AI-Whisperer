...
export function LiveWhisper() {
  ...
  // ⏱ Whisper generation loop (every ~8s)
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      // optional server call (works even without API key if /api/analyze fallback exists)
  ...
  const statusTone = !speechSupported ? "amber" : isRecording ? "green" : "gray";
  const statusText = !speechSupported ? "Mic not supported" : isRecording ? "Listening" : "Idle";
  const modeLabel =
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-semibold tracking-tight">Live Whisper</h1>
                <p className="text-sm text-gray-600">
                  Real-time micro-coaching while you speak — crisp phrasing, structure, confidence.
                </p>
              </div>
            </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-5">
          ...
          <div className="lg:col-span-3">
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
              <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold">What you’re saying</span>
                ...
                <div className="min-h-[260px] whitespace-pre-wrap rounded-xl bg-gray-50 p-4 text-sm leading-6 text-gray-800">
                  {transcript || (
                    <span className="text-gray-500">
                      Start the mic and speak naturally — your live transcript shows here.
                    </span>
                  )}
                </div>
              </div>
            ...
            {/* Quick coaching cards */}
            <div className="mt-6 grid gap-4 md:grid-cols-3">
              <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                <div className="text-xs font-semibold text-gray-700">Goal</div>
                <div className="mt-2 text-sm text-gray-600">
                  {persona === "Interview"
                    ? "Communicate impact + clarity"
                    : persona === "Standup"
                    ? "Align the team quickly"
                    : persona === "Sales Call"
                    ? "Drive to a clear next step"
                    : "Explain decisions + tradeoffs"}
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                <div className="text-xs font-semibold text-gray-700">Delivery</div>
                <div className="mt-2 text-sm text-gray-600">
                  Short sentences. One point at a time. Pause instead of filler words.
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
              <div className="border-b border-gray-100 px-5 py-4">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-semibold">Whispers</div>
                    <div className="text-xs text-gray-500">micro-coaching as you speak</div>
                  </div>
                  <button
                    onClick={() => setWhispers([])}
                    className="text-xs text-gray-500 hover:text-gray-700"
              ...
              <div className="max-h-[520px] overflow-auto px-5 py-4">
                {whispers.length === 0 ? (
                  <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 p-4 text-sm text-gray-600">
                    Your whispers will show up here once you start talking.
                  </div>
                ) : (
                      ...
                      .map((w, i) => (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <div className="mt-0.5 h-8 w-8 flex-shrink-0 rounded-xl bg-blue-100 text-blue-700 flex items-center justify-center">
                              💬
                            </div>
        ...
        <div className="mt-8 text-xs text-gray-500">
          Pro tip: finish each thought with a clear “next step” to sound decisive.
        </div>
      </div>
    </div>
  );
